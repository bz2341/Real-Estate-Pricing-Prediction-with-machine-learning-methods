...
class NeuralNetwork:
    '''神经网络'''
    def __init__(self, net_struct, mu = 1e-3, beta = 10, iteration = 200, tol = 0.1):
        '''初始化'''
...
if __name__ == '__main__':
    ...
    active_fun_list = ['sigm','sigm','sigm']# 【必须】设置【各】隐层的激活函数类型，
    #可以设置为tanh,radb，tanh,line类型，如果不显式的设置最后一层为line 
    ns = NetStruct(2, [10, 10, 10], 1, active_fun_list) # 确定神经网络结构，中间两个隐层各10个神经元
    nn = NeuralNetwork(ns) # 神经网络类实例化

#===========================================================  
    print(training_100.head())
    ...
    X0=testing_100.iloc[:,0:2]
    X0=X0.as_matrix()
    X0=X0.transpose()
    z0=testing_100.iloc[:,2]
    z0=z0.as_matrix()
    z0=z0.transpose()    
    ...
    plt.show()
    print("The test error is:%f"%test_perfermance(z0,z1))
#===========================================================
    ...
    X0=testing_1000.iloc[:,0:2]
    X0=X0.as_matrix()
    X0=X0.transpose()
    z0=testing_1000.iloc[:,2]
    z0=z0.as_matrix()
    z0=z0.transpose()    
...
#===========================================================
    ...
    X0=testing_10000.iloc[:,0:2]
    X0=X0.as_matrix()
    X0=X0.transpose()
    z0=testing_10000.iloc[:,2]
    z0=z0.as_matrix()
    z0=z0.transpose()    
